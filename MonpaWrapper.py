...
class MonpaWrapper:
    ...
    def handle_sentences(self, sentences, filter_pos_symbols=True, export_with_pos=True):
        ret = []
        for s in sentences:
    ...
    def handle_sentences_with_keys(self, payload, filter_pos_symbols=True, export_with_pos=True):
        ret = []
        for rec in payload:
            ret.append({'key': rec['key'], 'result': self.handle_single_sentence(rec['sentence'], filter_pos_symbols, export_with_pos)})
        return ret
